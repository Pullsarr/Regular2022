...
for i=1:length(b)
    ...
    subplot(2,1,2)
    plot(b(i).MoUSA(Czindex:length(a(i).data),1),b(i).MoUSA(Czindex:length(a(i).data),3),'--');
    hold on
    plot(b(i).MTOWUSA(Czindex:length(a(i).data),1),b(i).MTOWUSA(Czindex:length(a(i).data),3));
    hold on
    xlabel('V [kts]');
    ylabel('\gamma [\circ]');
    legend(VarNames);
