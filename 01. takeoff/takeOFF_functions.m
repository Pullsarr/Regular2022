classdef takeOFF_functions
    ...
    methods (Static)
        ...
        function obj=flight_mech(Config,Ciag,S,POS,Vod,gamma,mTOW)
            ...
            mi= takeOFF_functions.mi;
            rho=takeOFF_functions.rho;
            wind=takeOFF_functions.wind;
            Q=mTOW*g;
            Ps=interp1(Ciag.data(:,1),Ciag.data(:,2),POS.Vx+wind,"linear",'extrap')*g;
            ...
            Fy= (1/2)*rho*(POS.V+wind)^2*Cz*S;
            Fx= (1/2)*rho*(POS.V^2+wind)*Cx*S;
            if POS.V<Vod
                Rx=Ps-mi*Q-(1/2)*rho*(POS.Vx+wind)^2*S*Config.min.k*Drag_k;
                Ry=0;
            else
        ...
        function obj= start_anal(Config,Ciag,timeW,Lstart,MFV, MCV, Rot1_angle,Rot2_angle, Rot1_time, Rot1B_time, Rot2_time,Hmin,Lmax)
            ...
            for PAYLOAD=1:0.2:100
                ...
                for i=deltai:deltai:timeW
                    ...
                    if (POS.Lx>Lstart) || ((POS.V+wind)>Vod_min)
                        i1=i;
                        break
                    end
                end
        ...
        function obj = final_points(Config)
            ...
            Config.FS=Config.PAYLOAD(1)*2.20462/2+5;
